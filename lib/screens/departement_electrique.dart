...
class DepartementGEScreen extends StatefulWidget {
  const DepartementGEScreen({super.key});

  @override
  State<DepartementGEScreen> createState() => _DepartementGEScreenState();
}

class _DepartementGEScreenState extends State<DepartementGEScreen> {
  ...
  Future<void> _loadRooms() async {
    try {
      final data = await ApiService.getDepartmentRoomsByCode('E');
      if (mounted) setState(() {
                  ...
                  Column(
                    ...
                    children: [
                      Container(
                        ...
                        child: const Text('GE', style: TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w600)),
                      ),
                      const SizedBox(height: 4),
                      const Text('Génie Électrique', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
                      const Text('الهندسة الكهربائية', style: TextStyle(color: Colors.white70, fontSize: 13)),
                    ],
                  ),
                ],
              ),
  ...
  Widget _qrDeptButton(BuildContext context) {
    return GestureDetector(
      onTap: () => _showDeptQrDialog(context, 'E', 'Génie Électrique'),
      child: Container(
              ...
              Row(
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(color: _headerOrange.withOpacity(0.1), borderRadius: BorderRadius.circular(10)),
                    child: const Icon(Icons.qr_code_2_rounded, color: _headerOrange, size: 22),
                  ),
                    ...
                    MaterialPageRoute(
                      builder: (_) => RoomItemsScreen(
                        ...
                        department: const Department(
                          id: '', code: 'E', name: 'Génie Électrique', color: 'F97316'),
                      ),
                    ),
                  ),
        ...
        children: [
          ...
          Row(
            children: [
              Container(
                width: 48, height: 48,
                decoration: BoxDecoration(color: _headerOrange, borderRadius: BorderRadius.circular(14)),
                child: const Center(child: Text('LG', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16))),
              ),
              const SizedBox(width: 14),
              const Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Leila Gharbi', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1A2340))),
                  SizedBox(height: 2),
                  Text('Department Head — GE', style: TextStyle(fontSize: 13, color: Colors.black45)),
                ],
              ),
            ],
          ),
